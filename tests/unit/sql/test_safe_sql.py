...
async def test_select_statement(safe_driver, mock_sql_driver):
    ...
    mock_sql_driver.execute_query.assert_awaited_once_with("/* crystaldba */ " + query, params=None, force_readonly=True, page_size=100, offset=0)
...
async def test_complex_security_privilege_queries(safe_driver, mock_sql_driver):
    ...
    for query in queries:
        await safe_driver.execute_query(query)
        mock_sql_driver.execute_query.assert_awaited_with("/* crystaldba */ " + query, params=None, force_readonly=True, page_size=100, offset=0)
...
async def test_sql_driver_parameter_format(safe_driver, mock_sql_driver):
    ...
    mock_sql_driver.execute_query.assert_awaited_with(
        "/* crystaldba */ " + formatted_query, params=None, force_readonly=True, page_size=100, offset=0
    )
...
async def test_multiple_queries(safe_driver, mock_sql_driver):
    ...
    mock_sql_driver.execute_query.assert_has_awaits(
        [
            call("/* crystaldba */ " + query1, params=None, force_readonly=True, page_size=100, offset=0),
            call("/* crystaldba */ " + query2, params=None, force_readonly=True, page_size=100, offset=0),
        ]
    )
